...
try {
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['userInputStatus'])) {

        $status = filter_var($_POST['userInputStatus'], FILTER_SANITIZE_SPECIAL_CHARS);

        if (empty($status)) {
            throw new Exception("Status cannot be empty.");
        }

        $stmtInsert = $conn->prepare("INSERT INTO user_status (user_status_name) VALUES (?)");
        if ($stmtInsert === false) {
            throw new Exception("Failed to prepare the insert statement.");
        }

        $stmtInsert->bind_param("s", $status);
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'GET') {
        ...
        if (isset($_GET['statusId'])) {
            $statusId = base64_decode($_GET['statusId']);
            // Prepare the SQL statement to delete the vendor
            $stmtDelete = $conn->prepare("DELETE FROM user_status WHERE user_status_id = ?");
            $stmtDelete->bind_param("i", $statusId);
    ...
    if($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['editStatus'])) {
        $status = filter_var($_POST['editUserInputStatus'], FILTER_SANITIZE_SPECIAL_CHARS);
        ...
        $stmtUpdate = $conn->prepare("UPDATE user_status SET user_status_name = ? WHERE user_status_id = ?");
        if ($stmtUpdate === false) {
            throw new Exception("Failed to prepare the update statement.");
        }

        $stmtUpdate->bind_param("si", $status, $statusId);
                ...
                <div class="page-header">
                    <div class="add-item d-flex">
                        <div class="page-title">
                            <h4>Add/ Delete Status</h4>
                        </div>
                    </div>
                    <div class="page-btn">
                        <a href="#" class="btn btn-added" data-bs-toggle="modal" data-bs-target="#add-units"><i
                                data-feather="plus-circle" class="me-2"></i>Add Status</a>
                    </div>
                </div>
                                            ...
                                            <td class="action-table-data">
                                                <div class="edit-delete-action">
                                                    <a href="javascript:void(0);"
                                                        data-status-id="<?php echo $status['user_status_id']; ?>"
                                                    ...
                                                    <a href="javascript:void(0);"
                                                        data-status-id="<?php echo base64_encode($status['user_status_id']); ?>"
                                ...
                                <div class="row">
                                    <div class="col-lg-6">
                                        <div class="input-blocks">
                                            <label>Status *</label>
                                            <input type="text" name="userInputStatus" class="form-control"
                            ...
                            <form method="post" action="" id="myForm">
                                <div class="row">
                                    <div class="col-lg-6">
                                        <div class="input-blocks">
                                            <label>Status *</label>
                                            <input type="text" name="editUserInputStatus" class="userInputStatus form-control"
                                                placeholder="Enter username" required>
                                        </div>
                                    </div>
                                    <input type="hidden" name="statusId" class="statusId">
                                </div>
                                <div class="modal-footer-btn">
                                    <button type="button" class="btn btn-cancel me-2"
                                        data-bs-dismiss="modal">Cancel</button>
                                    <button type="submit" class="btn btn-submit" name="editStatus">Submit</button>
                                </div>
                            </form>
                        </div>
                    </div>
    ...
    <script>
        $(document).ready(function () {
            ...
            $('.editButton').on('click', function () {
                ...
                $.ajax({
                    url: 'fetch-status.php', // The PHP file that will handle the deletion
                    type: 'POST',
                    data: { statusId: statusId },
                    success: function (response) {
                        response.statusName.forEach((status) => {
                            $('.userInputStatus').val(status);
                        });
